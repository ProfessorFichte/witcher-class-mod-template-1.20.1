...
public class CustomSpells {
    public static void register() {
        ///////WITCHER_SPELLS
        /// AARD
        CustomSpellHandler.register(new Identifier(MOD_ID, "aard"), (data) -> {
            ...
            if (!data1.caster().getWorld().isClient) {
                List<Entity> list = data1.caster().getWorld().getOtherEntities(data1.caster(), data1.caster().getBoundingBox().expand(getSpell(new Identifier(MOD_ID, "aard")).range), selectionPredicate);
                for (Entity entity : list) {
                    SpellHelper.performImpacts(entity.getWorld(), data1.caster(), entity, entity, new SpellInfo(getSpell(new Identifier(MOD_ID, "aard")),new Identifier(MOD_ID)), data1.impactContext());
        ...
        /// AARD_SWEEP
        CustomSpellHandler.register(new Identifier(MOD_ID, "aard_sweep"), (data) -> {
            CustomSpellHandler.Data data1 = (CustomSpellHandler.Data) data;
            Predicate<Entity> selectionPredicate = (target2) -> {
                return (TargetHelper.actionAllowed(TargetHelper.TargetingMode.DIRECT, TargetHelper.Intent.HARMFUL, data1.caster(), target2)
                );
            ...
            if (!data1.caster().getWorld().isClient) {
                List<Entity> list = data1.caster().getWorld().getOtherEntities(data1.caster(), data1.caster().getBoundingBox().expand(getSpell(new Identifier(MOD_ID, "aard_sweep")).range), selectionPredicate);
                for (Entity entity : list) {
                    SpellHelper.performImpacts(entity.getWorld(), data1.caster(), entity, entity, new SpellInfo(getSpell(new Identifier(MOD_ID, "aard_sweep")),new Identifier(MOD_ID)), data1.impactContext());
                    if(entity instanceof PersistentProjectileEntity arrow){
                        Vec3d vec3d = arrow.getPos().subtract(arrow.getX(), arrow.getY(), arrow.getZ());
                        arrow.setVelocity(vec3d);
                    }
        ...
        /// QUEN_ACTIVE_SHIELD
        CustomSpellHandler.register(new Identifier(MOD_ID, "quen_active_shield"), (data) -> {
            CustomSpellHandler.Data data1 = (CustomSpellHandler.Data) data;
            if (!data1.caster().getWorld().isClient) {
        ...
        CustomSpellHandler.register(new Identifier(MOD_ID, "rend"), (data) -> {
            CustomSpellHandler.Data data1 = (CustomSpellHandler.Data) data;
            float adrenaline_damage_multiplier = (float) ((data1.caster().getAttributeValue(WitcherAttributes.ADRENALINE_MODIFIER)-100));
